...
uint8_t gc_execute_line(char *line)
{
  ...
  float parameter_data[N_AXIS]; // Multi-use variable to store parameter data for execution
	  ...
	  // NNW
	  uint8_t SendSpindelSpeedToPlaner = 0;
	  uint8_t SpindelSpeedParsed = 0;
  #endif
    ...
    switch(letter) {
      ...
      default:
        ...
        switch(letter){
          ...
          #ifdef VARIABLE_SPINDLE
          	case 'S': word_bit = WORD_S; gc_block.values.s = value; SpindelSpeedParsed = 1;break;
          #else
  ...
  switch(gc_block.non_modal_command) {
    ...
    case NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1:
      ...
      if (axis_command) {
          ...
          #ifdef VARIABLE_SPINDLE
			  // NNW
			  mc_line(gc_block.values.xyz, -1.0, false, -1);
          #else
        ...
        mc_line(parameter_data, -1.0, false, gc_block.values.n);
          ...
          #ifdef VARIABLE_SPINDLE
			  // NNW
			  mc_line(parameter_data, -1.0, false,-1);
          #else
    ...
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
      switch (gc_state.modal.motion) {
        ...
        case MOTION_MODE_LINEAR:
          ...
          #else
            #ifdef VARIABLE_SPINDLE
			  // NNW This is the place where we are going to plan the laser power
			  mc_line(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate, gc_block.values.s);
            #else
